...
/// (semitones) via the deterministic [PitchDiscriminationSession].
class PitchDiscriminationPage extends StatefulWidget {
  const PitchDiscriminationPage({
    super.key,
  ...
  final void Function(PitchDiscriminationSession session)? onCompleted;

  @override
  State<PitchDiscriminationPage> createState() =>
      _PitchDiscriminationPageState();
}

class _PitchDiscriminationPageState extends State<PitchDiscriminationPage> {
  late final PitchDiscriminationSession _session = PitchDiscriminationSession(
  ...
  Future<void> _present() async {
    ...
    final seq = buildOddballSequence(
      ...
      referenceHz: _session.referenceHz,
      deltaSemitones: _session.currentDeltaSemitones,
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Pitch discrimination'),
        actions: [
          IconButton(
            tooltip: 'Stop',
      ...
      children: [
        Row(
          children: [
            Chip(
              label: Text(
                'Δ ${_session.currentDeltaSemitones.toStringAsFixed(1)} st',
              ),
            ),
        ...
        Text('Play the three tones. One is higher — choose which.',
            style: theme.textTheme.titleMedium, textAlign: TextAlign.center),
        const SizedBox(height: 16),
        Center(
          child: Semantics(
            button: true,
            label: _playing ? 'Pause' : 'Replay tones',
            child: FilledButton.icon(
              ...
              label: Text(_playing
                  ...
                  : 'Replay tones ($_replays/$_maxReplays)'),
            ),
          ),
        ),
  ...
  Widget _buildResults(BuildContext context) {
    final theme = Theme.of(context);
    final threshold = _session.thresholdSemitones;
    return ListView(
      children: [
        ...
        _SummaryRow(
          label: 'Pitch-difference threshold',
          value: threshold == null
              ? 'not reached (needs more reversals)'
              : '${threshold.toStringAsFixed(2)}'
                  '${_session.track.thresholdSd == null ? '' : ' ± ${_session.track.thresholdSd!.toStringAsFixed(2)}'} semitones',
        ),
        const SizedBox(height: 14),
        NormTile(Norms.frequencySemitones(threshold)),
