...
import '../../core/training/timbre.dart';
...
/// Timbre-discrimination training: four waveforms (sine, triangle, sawtooth,
/// square) are played at the *same pitch* and identified in a 4AFC. As the
/// listener improves, the spectra are morphed toward each other so the timbres
/// become more similar. 20 adaptive trials.
///
/// Synthesized tones (`demo_only`). Adaptation changes only the spectral morph,
/// never master volume.
class TimbrePage extends StatefulWidget {
  const TimbrePage({
    super.key,
    this.audioPort,
    this.maxTrials = 20,
  ...
  @override
  State<TimbrePage> createState() => _TimbrePageState();
}

enum _Stage { training, results }

class _TimbrePageState extends State<TimbrePage> {
  late final AudioPort _audio = widget.audioPort ?? SilentAudioPort();
  late TimbreSession _session = TimbreSession(maxTrials: widget.maxTrials);
  late TimbreGenerator _generator = TimbreGenerator(seed: widget.seed);

  _Stage _stage = _Stage.training;
  TimbreTrial? _current;
  DateTime? _shownAt;
  bool _played = false;
  ...
  void _nextTrial() {
    setState(() {
      _current = _generator.next(_session.similarity);
  ...
  Future<void> _play() async {
    ...
    try {
      final samples = waveformTone(trial.target, widget.freqHz,
  ...
  void _choose(int index) {
    ...
    _records.add(TrialRecord(
      ...
      correct: correct,
      latencyMs: latency,
      parameters: <String, Object?>{'similarity': trial.similarity},
    ));
    setState(() {
      _chosen = index;
  ...
  Widget _buildTraining() {
    ...
    return TrialScaffold(
      title: 'Timbre training',
      subtitle: 'Which waveform?',
      instruction: 'Which waveform did you hear?',
      validationBadge:
      ...
      pills: [
        ...
        MetaPill(
            ...
            text: 'Blend ${(trial.similarity * 100).round()}%'),
      ],
      transport: [
        TransportAction(
  ...
  TrainingChoiceState _stateFor(int index, TimbreTrial trial) {
  ...
  Widget _buildResults() {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Timbre training')),
      body: Padding(
        ...
        child: ListView(
          children: [
            ...
            TrainingSummaryRow(
                label: 'Closest timbres told apart',
                value: '${(_session.bestSimilarity * 100).round()}% blend'),
            const SizedBox(height: 16),
            FilledButton(
              onPressed: () {
                setState(() {
                  _records.clear();
                  _session = TimbreSession(maxTrials: widget.maxTrials);
                  _generator = TimbreGenerator(seed: widget.seed);
                  _sw
                    ..reset()
                    ..start();
