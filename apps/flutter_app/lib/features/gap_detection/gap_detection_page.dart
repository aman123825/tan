...
///
/// Three noise intervals play in sequence; one contains a brief silent gap and
/// the listener chooses which. Difficulty adapts on gap duration (ms) via the
/// deterministic [GapDetectionSession]. The comfortable level is already LOCKED
/// before this screen; nothing here changes master volume. Temporal tasks are
/// wired-headphone recommended.
class GapDetectionPage extends StatefulWidget {
  ...
  /// Locked comfortable level in [0, 1] (display only; never changed here).
  final double comfortableLevel;

  final int maxTrials;

  /// Starting difficulty — sets the initial gap (ms) via [DifficultyConfig].
  /// Defaults to [DifficultyLevel.medium].
  final DifficultyLevel difficulty;

  final int seed;
  final String validationStatus;

  /// Audio output. Defaults to a silent port (used in tests/headless).
  final AudioPort? audioPort;

  /// Fired once when the run finishes (completed or stopped), for persistence.
  final void Function(GapDetectionSession session)? onCompleted;

  @override
  State<GapDetectionPage> createState() => _GapDetectionPageState();
}

class _GapDetectionPageState extends State<GapDetectionPage> {
  late final GapDetectionSession _session = GapDetectionSession(
    moduleId: widget.moduleId,
    groupId: widget.groupId,
    maxTrials: widget.maxTrials,
    track: AdaptiveTrack.gap(startMs: widget.difficulty.config.startGapMs),
  );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Temporal gap detection'),
        actions: [
          IconButton(
            tooltip: 'Stop',
  ...
  Widget _buildTrial(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          'Play the three sounds. One has a brief silent gap — choose which.',
  ...
  Widget _buildResults(BuildContext context) {
    final theme = Theme.of(context);
    final threshold = _session.thresholdGapMs;
    return ListView(
      children: [
        Text(
          'Session summary',
        ...
        _SummaryRow(
          ...
          value: threshold == null
              ...
              : '${threshold.toStringAsFixed(1)}'
                  '${_session.track.thresholdSd == null ? '' : ' ± ${_session.track.thresholdSd!.toStringAsFixed(1)}'} ms',
        ),
        const SizedBox(height: 14),
        NormTile(Norms.gapMs(threshold)),
          ...
          children: [
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Row(
                  children: [
                    Chip(label: Text('Gap ${gapMs.toStringAsFixed(1)} ms')),
                    ...
                    Chip(
                      ...
                      label: Text('Level: ${difficulty.label}'),
                    ),
                  ],
                ),
              ),
        ...
        children: [
          ...
          Expanded(
            child: Text(
              ...
              'are not suitable for gap thresholds.',
