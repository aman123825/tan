...
import '../../core/filtered_speech.dart';
...
/// Research-only, task-relative interpretation for a degraded-speech score
/// (typical ≥ 70% per ear on this task). Illustrative on uncalibrated audio.
NormResult degradedSpeechNorm(double? percent) {
  if (percent == null) return const NormResult.insufficient();
  const cite = 'Low-pass filtered-speech norm ≈ ≥ 70%/ear '
      '(task-relative, illustrative)';
  if (percent >= 85) {
    return const NormResult(NormBand.betterThanTypical,
        '≥ 85% per ear is strong performance on degraded speech.', cite);
  }
  if (percent >= 70) {
    return const NormResult(NormBand.withinTypical,
        '≥ 70% per ear is within the typical range for this task.', cite);
  }
  if (percent >= 55) {
    return const NormResult(NormBand.slightlyBelowTypical,
        '55–70% per ear is just below the typical range.', cite);
  }
  return const NormResult(NormBand.belowTypical,
      '< 55% per ear is below the typical degraded-speech range.', cite);
}

/// Low-Pass Filtered Speech Test renderer: each word is low-pass filtered
/// (default 1000 Hz) and presented monaurally; the listener types what they
/// heard. Scored per ear (typical ≥ 70%/ear).
class FilteredSpeechPage extends StatefulWidget {
  const FilteredSpeechPage({
    super.key,
  ...
  final void Function(FilteredSpeechSession session)? onCompleted;

  @override
  State<FilteredSpeechPage> createState() => _FilteredSpeechPageState();
}

class _FilteredSpeechPageState extends State<FilteredSpeechPage> {
  late final FilteredSpeechSession _session = FilteredSpeechSession(
    moduleId: widget.moduleId,
    groupId: widget.groupId,
    trialsPerEar: widget.trialsPerEar,
  );
  late final FilteredSpeechGenerator _generator = FilteredSpeechGenerator(
  ...
  Future<void> _play() async {
    ...
    try {
      ...
      final filtered = lowPassBiquad(decoded.samples, widget.cutoffHz,
          sampleRate: decoded.sampleRate);
      final wav = encodeMonauralWav(filtered,
          ear: trial.ear, sampleRate: decoded.sampleRate);
  ...
  void _submit() {
    ...
    final correct =
        _session.submit(trial, _input.text, latencyMs: latency, replays: _replays);
    setState(() {
      _answered = true;
  ...
  Widget build(BuildContext context) {
    if (_finished) {
      return Scaffold(
        appBar: AppBar(title: const Text('Filtered speech')),
        body: Padding(
    ...
    final ear = trial.ear == 'left' ? 'LEFT' : 'RIGHT';
    return TrialScaffold(
      title: 'Filtered speech',
      subtitle: 'Low-pass degraded speech',
      instruction: 'Listen to the muffled word in your $ear ear, then type it.',
      enableShortcuts: false,
      validationBadge:
          ValidationBadge(validationStatus: widget.validationStatus),
      pills: [
        MetaPill(icon: Icons.lock, text: 'Level $_levelPercent%'),
        MetaPill(icon: Icons.hearing, text: 'Ear: $ear'),
        MetaPill(
            icon: Icons.filter_alt,
            text: 'LPF ${widget.cutoffHz.toStringAsFixed(0)} Hz'),
      ],
      transport: [
        TransportAction(
  ...
  Widget _buildResults(BuildContext context) {
    ...
    return ListView(
      children: [
        ...
        Text('Left ear', style: theme.textTheme.labelLarge),
        const SizedBox(height: 6),
        NormTile(degradedSpeechNorm(
            _session.completedTrials == 0 ? null : _session.leftPercent.toDouble())),
        const SizedBox(height: 10),
        Text('Right ear', style: theme.textTheme.labelLarge),
        const SizedBox(height: 6),
        NormTile(degradedSpeechNorm(
            _session.completedTrials == 0 ? null : _session.rightPercent.toDouble())),
        const SizedBox(height: 12),
        Text(
            'Research measurement only — not a diagnosis. Low-pass filtered '
